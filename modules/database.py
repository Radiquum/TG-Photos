...
def update_DataBase():
    ...
    with engine.connect() as conn:
        conn.execute(text("CREATE TABLE posts (postId int, fileName str, tags str, day str, month str, year str, timestamp float)"))
        ...
        for item in sntelegram.TelegramChannelScraper(os.getenv('chatLink')).get_items():
            ...
            try:
                ...
                fileName = item.content.split("FileName: ")[-1].split("Taken:")[0]
            except (ValueError, IndexError):
                continue
            conn.execute(
                text("INSERT INTO posts (postId, fileName, day, month, year, timestamp) VALUES (:postId, :fileName, :day, :month, :year, :timestamp)"),
                [{"postId": postId, "fileName": fileName, "day": day, "month": month, "year": year, "timestamp": timestamp}],
            )
            conn.commit()
...
def add_to_DataBase(postId, fileName, takenTime, tags='NULL'):
    ...
    with engine.connect() as conn:
        conn.execute(
                text("INSERT INTO posts (postId, fileName, tags, day, month, year, timestamp) VALUES (:postId, :fileName, :tags, :day, :month, :year, :timestamp)"),
                [{"postId": postId, "fileName": fileName, "tags": tags, "day": day, "month": month, "year": year, "timestamp": timestamp}],
            )
        conn.commit()
...
def remove_from_DataBase(search_type, value):
    ...
    with engine.connect() as conn:
        conn.execute(text(f"DELETE FROM posts WHERE {search_type} == :{search_type}"), {search_type: value})
        conn.commit()
    return postId
